...
\section{Равномерная сходимость несобственного интеграла}
	...
	$$ f \ast g ~ (x) = \int_{R^m} f(x - t) g(t) d\lambda_m(t)$$
	
	\section{Интеграл Фурье, частичный интеграл Фурье}
	
	$f(x) = v.p. \lim\limits_{R \rightarrow \infty} \int\limits_{-R}^{R} \hat{f}(x) e^{2 \pi i x y} dy$ \\
	Частичный - только от $-\pi$ до $\pi$
	
	\section{Обратное преобразование Фурье}
	
	$f(x) = \int\limits_{R} \widehat{f}(y)e^{2\pi i<x,y>}dy -$ Формула обращения \\
$I_{A}(f,x) = \int\limits_{-A}^{A} \widehat{f}(y)e^{2\pi i<x,y>}dy $
...
$\mu$, $\nu$ - $\sigma$-конечны, полные \\
$m = \mu \times \nu$ \\
$f: \mathds{X} \times \mathds{Y} \rightarrow \overline{R}$, $f \geq 0$, f - суммируема относительно m \\
\emph{Тогда:}
\begin{enumerate}
	\item при \textit{почти всех} $x \in X$ $f_x$ - суммируема на $\mathds{Y}$, \\
	 где $f_x : \mathds{Y} \rightarrow \overline{R}$, $f_x(y) = f(x, y)$ \\
	 (симметричное утверждение верно для y)
	 \item Функция $x \mapsto \phi(x) = \int\limits_{\mathds{Y}}f_x d\nu = \int\limits_{\mathds{Y}}f(x, y) d\nu(y) $ - $\text{суммируема}^{\text{*}}$ на $\mathds{X}$ \\
	 (симметричное утверждение верно для y)
	 \item $\int\limits_{\mathds{X} \times \mathds{Y}}f(x, y)dm = \int\limits_{\mathds{X}}\phi(x)d\mu = \int\limits_{\mathds{X}}(\int\limits_{\mathds{Y}} f(x, y)d\nu(y))d\mu(x) = \int\limits_{\mathds{Y}}(\int\limits_{\mathds{X}}f(x,y)d\mu(x))d\nu(y)$
\end{enumerate}
...
\emph{Доказательство:}
    ...
    $f: Y \subset \widetilde{Y}, \ X * Y \rightarrow \overline R, \ y_0 - $ предельная точка для $ Y, \ y_0 \in \widetilde{Y} $ 
    \begin{enumerate}
    	\item при п.в. $ x \ \exists f_0(x) = \underset{y \rightarrow y_0}{\lim} f(x,y) $
    	\item $ f_0 - $ сумм на всех пр-ах вида $ <0, t>, \ t < b $ и верно : $ \int\limits_a^t f(x, y) \underset{y \rightarrow y_0}{\rightarrow} \int\limits_a^t f_0(x)dx $
    	\item несобств инт $ I(y) = \int\limits_a^{\rightarrow b} f(x,y)dx - $ равномерное сходится при $ y \in Y$ \\
    \end{enumerate}
    
    \emph{Тогда}
    	Несобственный интеграл $ \int\limits_a^{\rightarrow b} f_0(x)dx - $ существует и $ I(y) \underset{y \rightarrow y_0}{\rightarrow} \int\limits_a^{\rightarrow b} f_0(x)dx $

    \section{Вычисление интеграла Дирихле}
    Ты проиграл

    \section{Теорема об интегрировании несобственного интеграла по параметру}
    $ f : (a,b) * Y \rightarrow \overline{R}, \ f - $ сумм по мере $ \lambda_1 * \mu $ на мн-вах $ (a,t)*Y, \ a < t < b \\
    J(y) = \int\limits_a^{\rightarrow b} f(x,y)d\lambda_1(x) - $ равн сх на $ Y $ \\
    $ I(x) = \int\limits_Y f(x,y)\mu(y)$ \\
    \emph{Тогда} 
    $ J - $ сумм на $ Y $ и $ \int\limits_a^{\rightarrow b} I(x)d\lambda_1(x) - $ сх \\
    $\int\limits_Y J(y)d\mu = \int\limits_a^{\rightarrow b} I(x)d\lambda_!(x)$

    \section{Правило Лейбница для несобственных интегралов}
    $ f: [a,b] * <c, d> \rightarrow R - $ непр $ \ \forall y \in <c,d> : \\
    J(y) = \int\limits_a^{\rightarrow b} f(x,y)dx - $ известно, что сх-ся \\
    $ \forall x, \ \forall y \ \exists f_y'(x,y) - $ непр на $ [a.b] * <c,d> \\
    I(y) = \int\limits_a^{\rightarrow b} f_y'(x,y)dx - $ равн сх-ся по $ y \in <c,d> $ \\
    \emph{Тогда}
    $ J \in C^1<c,d> $ и $ J'(y) = I(y) $ т.е. \\
    $ (\int\limits_a^{\rightarrow b} f(x,y)dx)_y' = \int\limits_a^{\rightarrow b} f_y'(x,y)dx $

    \section{Теорема о вычислении интеграла по мере Бореля---Стилтьеса (с леммой)}
    \subsection{Лемма}
    $ h : X \rightarrow \overline{R} - $ изм, п.в. конечна, $ H(t) - $ функция распределения \\
    \emph{Тогда}
    $\mu_H - $ мера Бореля-Стилтьеса на $ (R, B) $ совпадает с мерой $ h(\mu) \ \ (\nu(A) = \mu(h^{-1}(A))$
	\subsection{Теорема}
	$ f: R \rightarrow R, \ f \geq 0, $ изм по Борелю \\
	$ h : x \rightarrow \overset{R}, $ изм п.в., кон $ H(t) - $ ф-я распределения, $ \mu_H - $ мера Бореля-Ститьеса \\
	\emph{Тогда}
	$ \int\limits_X f(h(x))d\mu(x) = \int\limits_R f(t)d\mu_H(t) $ 
...
\textbf{\emph{Замечание:}} $ L_{\infty} $ -- полное (упражнение)
    ...
    Множество ступ функкций плотно в $ L_p(X, \mu)\ \ (\phi - $ ступ ф-я, $ \phi \in L_p \Rightarrow \mu X (\phi \neq 0) < + \infty)$
...
$|\int\limits_{\mathds{R}}f(t)e(kt)| \leq\frac{1}{2}\int\limits_{\mathds{R}}|f(t) - f(t + \frac{\pi}{k})|dt$ (так как $|e(kt)| \leq 1$)$ \xrightarrow[k \to +\infty]{}0$,\\по непрерывности сдвига
    ...
    $|a_k(f)|, \ |b_k(f)|, \ 2|c_k(f)| \leq w(f, \frac{\pi}{k})$
    \subsection{Следствие 2}
    $f \in \widetilde{C^r}[-\pi, \pi] $ \\
    \emph{Тогда}
    $|a_k(f)|, \ |b_k(f)|, \ 2|c_k(f)| \leq \frac{const}{|k|^r} \ \ (k \neq 0) $
    \subsection{Следствие 3}
    \begin{enumerate}
	\item     $ 0 < \alpha \leq 1, \ f \in Lip_{\mu} \alpha $ \\
    \emph{Тогда}
    $|a_k(f)|, \ |b_k(f)|, \ 2|c_k(f)| \leq \frac{\mu \pi^{\alpha}}{|k|^r} $
    
    \item     $ 0 < \alpha \leq 1, f \in C^r, \ f^r \in Lip_{\mu} \alpha $ \\
    \emph{Тогда}
    $|a_k(f)|, \ |b_k(f)|, \ 2|c_k(f)| \leq \frac{const}{|k|^{r + \alpha}} $    
    \end{enumerate}
...
\end{enumerate}
    ...
    $ f \in L^1[-\pi, \pi] \Rightarrow \sum\limits_{n = 1}^{+ \infty} \frac{b_n(f)}{n} - $ сх-ся
...
\emph{Определение:} $f, f_n$ -- последовательность обобщенных функций: $f_n \rightarrow f$, если $\forall u \in \mathbb{C}^\infty \quad \int\limits_{-\pi}^{\pi} f_n u \rightarrow \int\limits_{-\pi}^{\pi} f u$.
    ...
    \begin{enumerate}
    \item $ f \in \widetilde{C^1}[-\pi, \pi] \Rightarrow $ ее суммы Фурье равн огр, т.е \\
    $ \exists C : \forall n, \ \forall x \ |S_n(f, x)| < C $
    \item $ f \in \widetilde{C^1}[-\pi, \pi], \ g - $ изм, гл, пер, огр с равномерно огр суммами Фурье, $ \exists C : \forall n, \ \forall x \ |S_n(f, x)| < C $ \\
    \emph{Тогда}
    $\int\limits_{-\pi}^{\pi} f \overline{g} dx = 2\pi\sum c_n(f) * \overline{c_n(g)} - $ обобщ рав-во Пасеваля
    \end{enumerate}
...
\begin{enumerate}
	\item $ W_t - $ а.е. при $ t \rightarrow 0 $
	\item $ W_t(x) = \int\limits_{R^m} e^{-\pi  t^2 |y|^2} e^{-2 \pi i <x, y>} dy $ т.е. $ V_t = e^{-\pi t^2 |x|^2} $ тогда $ W_t = (V_t)\hat{} $
\end{enumerate}

\subsection{а.е Вейерштрасса}
$ W_t(x) = \frac{1}{t^m}e^{-\frac{\pi}{t^w}x^2} = \frac{1}{t^m}e^{-\frac{\pi}{t^w}(x_1^2 + ... + x_m^2)}, \ x \in R^m \\$
...
$\hat{} : S \rightarrow S - $ взаимоодназанчно

\subsection{Класс быстро убывающих функций}
	$ S(R^m) = \{f \in C^\infty(R^m) : \forall \alpha, \beta $ - мультииндексы $ \underset{x \in R^m}{sup|x^\beta D^\alpha f(x)| < +\infty}\} $

\section{Теорема Вейерштрасса (лемму можно не доказывать)}

$ f \in C(R^m) \ \forall R, \ \forall\epsilon > 0 \ \exists $ полином $ P(x) = P(x_1,...,x_m) : |f(x) - P(x)| < \epsilon $ при $ x \in B(0, R)$

	\subsection{Лемма} 
	\begin{enumerate}
	\item $ f - $ финитная в $ R^m, Q - $ многочлен
		\emph{Тогда}
		$ f * Q - $ многочлен
	\item $ W_t = \frac{1}{t^m}e^{-\frac{\pi}{t^2}|x|^2}, \ x \in R^m - $ это функция хорошо приьлижающихся многочленов в любом шаре \\
		$\forall R \ \forall n \exists $ многочлен $ P_n : \\
		\forall x \in B(0, R) \ |W_t(x) - P_n(x)| \leq \frac{1}{t^m n!}(\frac{\pi R^2}{t^2})^n $
	\end{enumerate}
	
	\emph{Доказательство}
	$f_t = f * W_t, \ f_t \underset{t \rightarrow 0}{\Rightarrow} f, $ фикс $ t \ |f(x) - f_t(x)| < \epsilon$ \\
	\textcolor{red}{TODO}
...
\textcolor{red}{Возможно не то следствие}

$ f \in C^r(R^m)$
\emph{Тогда}
$\forall R > 0, \ \forall \epsilon > 0 \ \exists P $ (многоч) $ \forall$ мультииндекса $ k : |k| \leq r : \\
|D^kf(x) - D^kP(x)| < \epsilon, \ x \in B(0, R)$ 
