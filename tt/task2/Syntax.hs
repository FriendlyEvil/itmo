...
happyExpList = Happy_Data_Array.listArray (0,36) ([1408,80,5,1408,0,22528,1024,4,16384,512,16384,22528,0,88,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Expression","Application","Atom","Dot","Slesh","LEFT","RIGHT","VARIABLE","%eof"]
        bit_start = st * 12
        bit_end = (st + 1) * 12
...
action_14 (7) = happyShift action_17
...
happyReduction_2 ((HappyAbsSyn4  happy_var_5) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (TokenVariable happy_var_3)) `HappyStk`
	_ `HappyStk`
	(HappyAbsSyn5  happy_var_1) `HappyStk`
	happyRest)
	 = HappyAbsSyn4
		 (ApplLambda happy_var_1 (Var happy_var_3) happy_var_5
	) `HappyStk` happyRest
...
happyReduction_3 ((HappyAbsSyn4  happy_var_4) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (Lambda (Var happy_var_2) happy_var_4
	) `HappyStk` happyRest
...
happyReduction_7 (HappyTerminal (TokenVariable happy_var_1))
	 =  HappyAbsSyn6
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenDot -> cont 7;
	TokenSlesh -> cont 8;
	TokenLeftBracket -> cont 9;
	TokenRightBracket -> cont 10;
