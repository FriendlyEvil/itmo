...
class HttpServer {
    ...
    private Map<File, String> etag = new HashMap<>();
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file = new File(root, java.net.URLDecoder.decode(request.getUri(), "UTF-8"));
        if (file.isFile()) {
            read(response, file, request.getHeader("If-None-Match"));
    ...
    private void read(Response response, File file, String headers) throws IOException {
        if (headers != null && headers.equals(MD5(file))) {
            response.setStatusCode(304);
            response.setHeader("ETag", String.valueOf(etag.get(file)));
            return;
        }

        etag.put(file, MD5(file));
    ...
    private String MD5(File file) throws IOException {
        try {
            ...
            byte[] array = md.digest(Files.readAllBytes(Paths.get(file.getPath())));
