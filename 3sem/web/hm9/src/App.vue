        ...
        components: {
            ...
            this.$root.$on("onEnter", (login, password) => {
                ...
                if (users.length) {
                    if (users[0].password === password) {
                        ...
                        this.$root.$emit("onEnterValidationError", "Invalid password.");
                    }
                } else {
                    this.$root.$emit("onEnterValidationError", "Invalid login.");
                }
            });
            ...
            this.$root.$on("onRegister", (name, login, password) => {
                if (!name || name.length < 1 || name.length > 32) {
                    this.$root.$emit("onAddPostValidationError", "Name is invalid");
                } else if (!login || login.length < 3 || login.length > 16) {
                    this.$root.$emit("onAddPostValidationError", "Login is invalid");
                } else if (!password || password.length < 3 || password.length > 32) {
                    this.$root.$emit("onAddPostValidationError", "Password is invalid");
                } else {
