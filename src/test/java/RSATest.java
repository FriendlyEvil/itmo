...
public class RSATest {
  ...
  private void textTest(String message) {
    ...
    BigInteger encodeMessage = RSA.encode(mess, openKey);
  ...
  private void integerTest(BigInteger message) {
    ...
    BigInteger encodeMessage = RSA.encode(message, openKey);
    BigInteger decodeMessage = server.decode(encodeMessage);

    //TODO fix it
    Assert.assertEquals(decodeMessage, message.mod(openKey.getSecond()));
  }
